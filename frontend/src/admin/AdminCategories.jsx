...
const AdminCategories = () => {
    ...
    const onSave = async (e) => {
        ...
        try {
            if (editing.id) {
                await updateCategory(editing.id, { name: editing.name.trim(), icon: editing.icon });
                toast.show("Đã cập nhật danh mục.", "success");
            } else {
                const slug = editing.name.toLowerCase()
                    .normalize("NFD").replace(/[̀-ͯ]/g, "")
                    .replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "");
                await addCategory({ id: slug || `cat-${Date.now()}`, name: editing.name.trim(), icon: editing.icon });
